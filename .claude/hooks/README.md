...
`CPP_GATES_OVERRIDE=1`, and every use is written to the event log as a `gate_override`
row, so the bypass rate is measurable rather than silent.
...
# Should exit 2 (story-writer entered with requirements gate pending)
